  ...
  <modal name="new-project" classes="p-10 bg-card rounded-lg" height="auto">
    ...
    <form @submit.prevent="submit">
      ...
      <div class="flex">
        <div class="flex-1 mr-4">
          <div class="mb-4">
            ...
            <input type="text"
                   id="title" name="title"
                   class="block border p-2 text-xs rounded w-full"
                   :class="form.errors.title ? 'border-error' : 'border-muted-light'"
                   v-model="form.title">
            <span class="text-xs text-error" v-if="form.errors.title" v-text="form.errors.title[0]"></span>
          </div>
          ...
          <div class="mb-4">
            ...
            <textarea type="text"
                      ...
                      class="block border border-muted-light p-2 text-xs rounded w-full" rows="5"
                      v-model="form.description"
                      :class="form.errors.description ? 'border-error' : 'border-muted-light'"></textarea>
            <span class="text-xs text-error" v-if="form.errors.description" v-text="form.errors.description[0]"></span>
          </div>
        </div>
        ...
        <div class="flex-1 ml-4">
          <div class="mb-4">
            ...
            <input type="text"
                   class="block border border-muted-light mb-2 p-2 text-xs rounded w-full"
                   placeholder="Task 1"
                   v-for="task in form.tasks"
                   v-model="task.body">
          </div>

          <button type="button" class="inline-flex items-center text-xs mb-8" @click="addTask">
      ...
      <footer class="flex justify-end">
        <button type="button" class="button is-outline mr-4" @click.prevent="$modal.hide('new-project')">Cancel</button>
